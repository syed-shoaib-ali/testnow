...
class QuestionRadio extends Component {
  ...
  async componentDidMount() {
    ...
    this.candidateInterval = setInterval(() => {
      this.candidateHeartBeat();
    }, 5000);
    ...
    if (isResume && minsAgo < allowResumeTestMinutes) {
      ...
      const liveFeedData = {
        type: "login",
        name: this?.state?.keySno?.toString(),
      };

      conn.send(JSON.stringify(liveFeedData));
    ...
    axios({
      ...
      .then(({ data }) => {
        ...
        const liveFeedData = { type: "login", name: info?.keySno?.toString() };
        conn.send(JSON.stringify(liveFeedData));
      })
      .catch((e) => {
        // alert(e);
  ...
  componentWillUnmount() {
    clearInterval(this.candidateInterval);
  ...
  candidateHeartBeat = () => {
    if (this.state.keySno !== "") {
      const url =
        baseURL +
        "/userpingtoserverajax/" +
        this.state.compCode +
        "?keySno=" +
        this.state.keySno;
      const headers = {
      ...
      axios({ method: "POST", url, headers }).catch((e) => {
        console.log(url, " HEARTBEAT ERROR ", e?.response?.data || e);
      });
    }
  };
      ...
      <View style={styles.MainView}>
        ...
        {QUESTION != null ? (
          <>
              ...
              <Card style={styles.circleCard}>
                ...
                liveFeed ? (
                  <WebRTCCamera
                    ...
                    cameraStyle={styles.circleCard}
                    userId={(keySno && keySno.toString()) || "Unknown Keysno"}
                    setLiveFeed={(liveFeed) => this.setState({ liveFeed })}
                    liveFeed={liveFeed}
                    activeSocket={socketActive}
                    conn={conn}
                    callOffer={callOffer}
                    callName={callName}
                    candidate={candidate}
            ...
            <Text>{liveFeed ? "Live Feed" : "Camera"}</Text>
            ...
            <View style={styles.absoluteViewWrap}>
              ...
              <View style={styles.GreenBtns}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    this.props.navigation.push("QuestionRadioMesseges", {
                      keySno,
                      email,
                      wheeboxId,
                    })
